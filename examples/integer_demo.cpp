...
int main(int argc, char* argv[]) {
    argsparser::Parser parser("integer_demo", "Demonstration of new integer argument types");

    // Add arguments for different integer types
    auto* regular_int = parser.addArgument<int>("int", "i", "Regular signed integer", false, 0);
    auto* unsigned_int = parser.addArgument<unsigned int>("uint", "u", "Unsigned integer", false, 0u);
    auto* long_int = parser.addArgument<long>("long", "l", "Long signed integer", false, 0L);
    auto* unsigned_long = parser.addArgument<unsigned long>("ulong", "g", "Unsigned long integer", false, 0UL);
    ...
    std::cout << "Unsigned long value: " << unsigned_long->getValue() << std::endl;
